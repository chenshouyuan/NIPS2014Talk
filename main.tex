...
\begin{frame}[plain]
 ...
 \begin{center}
  ...
  {$^1$ CUHK \,\, $^2$ Tsinghua University \,\, $^3$ Microsoft Research Asia}
...
\begin{frame}{Combinatorial Pure Exploration of MAB}
  \begin{block}{Combinatorial Pure Exploration (CPE)}
    ...
    \begin{itemize}
      ...
      \vspace{-1em}
              $$M_* = \argmax_{\textcolor{DarkRed}{M\in \M}} \sum_{i\in M} w(i)$$
      \vspace{-1em}
      \begin{itemize}         
...
\begin{frame}{Our Results}
  \begin{itemize}
    \item<1-> \textcolor{DarkRed}{algorithms}
      \begin{itemize}
        \item two general learning algorithms for a wide range of $\M$.
      \end{itemize}
    \item<2-> \textcolor{DarkRed}{upper bounds}
      \begin{itemize}
        \item sample complexity / probability of error.
        %\item \textbf{exchange class}: a new tool for analysis.
      \end{itemize}
    \item<3-> \textcolor{DarkRed}{lower bound}
      \begin{itemize}
        \item algorithms are \alert{optimal} (within log factors) for many types of $\M$ (in particular, bases of a matroid).
      \end{itemize}
    \item<4-> \textcolor{DarkRed}{compared with existing work}
      \begin{itemize}
...
\begin{frame}{Width and exchange class}
  \begin{exampleblock}{Intuitions}
    \begin{itemize}
      ...
      \begin{itemize}
        \item an exchange class is a ``proxy'' for the structure of $\M$.
      \end{itemize}
      \item an exchange class is a collection of ``patches'' that are used to interpolate between valid sets.
...
\begin{frame}[noframenumbering]{CSAR: Fixed budget algorithm}
  ...
  \begin{itemize}
    ...
    \end{itemize}
    \item<3-> recall the (unknown) \textbf{gap} of arm $e$:
      $$        
          \Delta_e = \begin{cases}
             w(M_*)-\max_{M\in \M: e\in M} w(M) & \text{if } e\not \in M_*, \\
             w(M_*)-\max_{M\in \M: e\not \in M} w(M) & \text{if } e\in M_*
...
\begin{frame}{Summary}
\begin{itemize}
  \item combinatorial pure exploration: a general framework that covers many pure exploration problems in MAB.
    \begin{itemize}
      \item find top-$k$ arms, optimal spanning trees, matchings or paths.
    \end{itemize}
  \item two general algorithms for the problem
    \begin{itemize}
...
\begin{itemize}
  ...
  \item support approximate maximization oracles
  \item non-linear rewards
\end{itemize}
\end{frame}
